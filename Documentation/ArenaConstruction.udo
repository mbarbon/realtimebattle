...
if a collision has occured. Circle and inner_circle prevents robots from entering into a circle or
getting outside 
a circle respectively. The line stops the robots from entering through the long side, but no check is
made on the short side, so you have to put circles at each end of the line to make it a solid
...
Note that RealTimeBattle does no check on whether the arena file gives a correct arena, it is
...
determines the material of the wall. They are both values between 0 and 1. Harder walls will injure
...
!begin_description
   ...
   !item [exclusion_point [position_x!][position_y!]]
         When the arena inside the boundary consists of several separated areas, you should exclude
         all but one by 
         inserting exclusion points. All points from which you can draw a straight line to an
         exclusion point without crossing a wall, are considered to be outside the arena.
